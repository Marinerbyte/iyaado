...
# --- AUTO FONT DOWNLOADER (New Feature) ---
# Ye function khud font download kar lega agar nahi mila to
def check_and_download_font():
    ...
    if not os.path.exists(font_path):
        ...
        try:
            # Google Fonts se direct download link
...
async def start_bot_main_loop():
    ...
    while bot_config["is_running"]:
        try:
            async with websockets.connect(SOCKET_URL, ssl=True, extra_headers={'User-Agent': 'Mozilla/5.0'}) as websocket:
